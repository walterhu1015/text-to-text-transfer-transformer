...
def get_valid_tasks(tasks, split):
  """Get tasks that have the specified split and metric function."""
  for task in tasks:
    ...
    if not task.metric_fns:
      logging.info("Skipping %s because metric_fns is empty", task.name)

  tasks_with_split = [task for task in tasks if split in task.splits]
  valid_tasks = [task for task in tasks_with_split if task.metric_fns]

  return valid_tasks


def get_targets_and_examples(tasks, dataset_fn, eval_summary_dir):
  """Get targets and examples.

  Args:
    tasks: list, contains tasks objects.
    dataset_fn: function, returns the dataset from the task object.
    eval_summary_dir: str, directory to store eval summaries.
  Returns:
    List of examples, list of targets from the dataset, and a dict with max
  ...
  for task in tasks:
    ...
    for ex in tfds.as_numpy(ds):
      ...
      examples.append(ex)
      inputs.append("inputs_plaintext")
  ...
  return cached_examples, cached_targets, max_sequence_length
